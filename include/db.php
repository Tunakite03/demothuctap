...
class db
{
	...
	// private function setNames($encode = 'utf8')
	// {
	// 	mysqli_set_charset($this->link, $encode);
	// }
	...
	public function record_insert($table_name, $data)
	{
		...
		foreach ($data as $key => $value) {
			$columns[] = '`' . $key . '`';
			$values[] = "'" . mysqli_real_escape_string($this->link, htmlspecialchars_decode(str_replace("'", "\'", $value))) . "'";
	...
	public function record_update($table_name, $data, $where = false)
	{
		...
		$fields = array();
		foreach ($data as $key => $value) {
			$fields[] = " `$key`= '" . mysqli_real_escape_string($this->link, htmlspecialchars_decode($value)) . "'";
	...
	{
		return mysqli_real_escape_string($this->link, htmlspecialchars_decode($str));
	}
