...
class main
{
	...
	public function convert_strdate($date)
	{ //strdate format d/m/Y
	...
	private function doi3so($number, $dictionary, $doc = '')
	{ //#$doc khi giá trị là 1 thì đọc còn ko thì trả về rỗng
		$number = floatval($number);
		if ($number == 0) {
			if ($doc == '1') {
				return '';
	...
	function upimg($file, $dir, $x = "1200")
	{
		...
		if ($handle->uploaded) {
			...
			if ($handle->image_src_x > $x || $handle->image_src_y > $x) { //Chỉ resize khi image vượt mức quy định, không thì thôi
				if ($handle->image_src_x > $handle->image_src_y) { //nếu x wide dài hơn thì resize x
	...
	public function mySubString($string, $lenght)
	{ //sub string from Start: avoid unicode word
	...
	public function get_youtube_info($url)
	{ //Get youtube info by link
	...
	public function getUnixtimeTheQuater($unixtime = 0)
	{ ///HuanCoder => 1/4/21 => lấy thời gian bắt đầu và kết thúc của 1 quí
